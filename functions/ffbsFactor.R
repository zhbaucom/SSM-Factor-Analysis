ffbs.joint = function(y,G,V,W,m0,C0, timeDiff = rep(1, ncol(y))){
  ...
  mm[,T] = m[,T]; CC[,,T] = C[,,T]
  x[,T] = mvtnorm::rmvnorm(1, mean = m[,T], sigma = (C[,,T]), checkSymmetry = FALSE) 
  ...
  for (t in (T-1):1){ 
    ...
    x[,t] = mvtnorm::rmvnorm(1,mean = m[,t]+B[,,t]%*%(x[,t+1]-a[,t+1]), sigma = H[,,t], checkSymmetry = FALSE)
  } 
